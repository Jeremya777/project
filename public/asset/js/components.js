...
(function() {
  ...
  function resetAdaptNav() { // reset nav appearance
    ...
    for(var i = 0; i < this.outrunIndex; i++) {
      if( Util.hasClass(this.items[i], 'hidden')) {
        Util.addClass(this.items[i], 'adapt-nav__item--hidden');
        Util.removeClass(this.items[i], 'hidden');
      }
      ...
      if(right >= maxPosition || hideAll) {
        ...
        Util.addClass(this.items[i], 'hidden');
        hideAll = true;
      } else {
        Util.removeClass(this.items[i], 'hidden');
      }
  ...
  function resetOutrun(nav, cloneList, bool) {
    if(nav.items[0].offsetLeft < 0 || (bool && nav.outrunIndex > 1)) {
      ...
      Util.addClass(nav.items[nav.outrunIndex], 'hidden');
  ...
  function modifyClone(clone) { // assign new classes to cloned elements inside the dropdown
    ...
    Util.removeClass(clone, 'js-adapt-nav__item hidden adapt-nav__item--hidden adapt-nav__item');
...
(function() {
  ...
  function getOptionsList(select, options) {
    ...
    for(var i = 0; i < options.length; i++) {
      ...
      list = list + '<li><button type="button" class="js-select__item select__item select__item--option" role="option" data-value="'+options[i].value+'" '+selected+' data-index="'+select.optionIndex+'">'+options[i].text+'</button></li>';
  ...
  Rating.prototype.initRatingHtml = function() {
    ...
    if(!this.readOnly) {
      ...
      if(labelElement.length > 0) {
        ...
        Util.addClass(labelElement[0], 'hidden');
      }
    }
